...
import { ArrowLeft, Sparkles, PlusCircle, Users, X, Check, UserPlus, LogOut } from 'lucide-react';
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(6px)', zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
      <div className="card animate-fade-up" style={{ width: '100%', maxWidth: 460, padding: '2rem', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
          {EMOJIS.map(e => (
            <button key={e} onClick={() => setEmoji(e)} style={{ fontSize: '1.5rem', width: 44, height: 44, borderRadius: 'var(--radius-md)', border: `2px solid ${emoji === e ? 'var(--primary)' : 'transparent'}`, background: emoji === e ? 'rgba(91,94,244,0.1)' : 'var(--surface-low)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all var(--transition)' }}>
              {e}
            </button>
          ))}
        </div>
    ...
    <div className="page animate-fade-up">
      ...
      <div className="desktop-grid-2" style={{ marginBottom: '2rem' }}>
        <div className="card" style={{
          ...
          border: `1.5px solid ${group.net_balance >= 0 ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}`,
        }}>
          <p className="text-muted" style={{ fontSize: '0.8125rem', marginBottom: '4px' }}>
            {group.net_balance >= 0 ? 'You are owed' : 'You owe'}
          </p>
          <p style={{ fontSize: '2.25rem', fontWeight: 800, color: group.net_balance >= 0 ? 'var(--success)' : 'var(--error)' }}>
            ₹{Math.abs(group.net_balance).toLocaleString('en-IN')}
          </p>
          <p className="text-muted" style={{ fontSize: '0.8125rem', marginTop: '8px' }}>
            Overall balance in {group.name}
          </p>
        </div>

        {balances?.length > 0 && (
          <div className="card">
            <p style={{ fontWeight: 600, fontSize: '0.875rem', marginBottom: '1rem' }}>Member Balances</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {balances.map(b => (
                <div key={b.user_id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <span style={{ fontWeight: 700, color: b.balance >= 0 ? 'var(--success)' : 'var(--error)', fontSize: '0.9375rem' }}>
      ...
      <div className="section-header">
        <h3>Expenses ({group.expenses?.length ?? 0})</h3>
            ...
            <div key={e.id} className="expense-row">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p className="text-muted" style={{ fontSize: '0.8125rem' }}>
                  Paid by {e.paid_by_name} · {e.split_type === 'equal' ? 'split equally' : 'custom'}
                </p>
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
